  ...
  test.describe('Desktop (≥1024px)', () => {
    ...
    test('搜索框可见', async ({ page }) => {
      ...
      const searchInput = page.locator(
        'input[type="search"], input[placeholder*="搜索"], [class*="search"] input',
      )
  ...
  test.describe('Mobile (<768px)', () => {
    ...
    test('点击展开侧边栏/下拉菜单', async ({ page }) => {
      ...
      const hamburger = page
        .locator('[class*="hamburger"], [class*="menu-toggle"], button[aria-label="menu"]')
        .first()
    ...
    test('菜单项可点击', async ({ page }) => {
      ...
      if (await hamburger.isVisible()) {
        ...
        // 获取菜单项
        const nav = page
          .locator('[class*="mobile-menu"], [class*="sidebar"], aside, nav[class*="mobile"]')
          .first()
        const menuItems = nav.locator('a, button')
    ...
    test('关闭按钮正常工作', async ({ page }) => {
      ...
      if (await hamburger.isVisible()) {
        ...
        const closeButton = page
          .locator('[class*="close"], button[aria-label="close"], [class*="dismiss"]')
          .first()
        ...
        if (await closeButton.isVisible()) {
          ...
          const mobileMenu = page
            .locator('[class*="mobile-menu"], [class*="sidebar"], aside')
            .first()
    ...
    test('点击外部关闭菜单', async ({ page }) => {
      ...
      if (await hamburger.isVisible()) {
        ...
        // 检查菜单是否关闭
        const mobileMenu = page.locator('[class*="mobile-menu"], [class*="sidebar"], aside').first()
