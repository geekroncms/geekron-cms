...
export async function fillForm(page: Page, selector: string, data: Record<string, string>) {
  for (const [key, value] of Object.entries(data)) {
    const input = page.locator(`${selector} [name="${key}"], ${selector} input#${key}, ${selector} textarea#${key}`);
    await input.fill(value);
  }
}

// Mock API 响应
export async function mockAPI(page: Page, method: string, url: string, response: any, status = 200) {
...
export async function checkValidationError(page: Page, fieldName: string, errorMessage: string) {
  const errorLocator = page.locator(`[name="${fieldName}"] + [class*="error"], [class*="error"]:has-text("${errorMessage}")`);
