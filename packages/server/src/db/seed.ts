...
export async function seedDatabase(db: D1Database) {
  ...
  `,
    ...
    .bind(tenantId, 'Default Tenant', 'default', 'default@example.com', 'pro', 'active')
    .run()
  ...
  `,
    ...
    .bind(adminId, tenantId, 'admin@example.com', adminPassword, 'Admin User', 'owner', 'active')
    .run()
  ...
  `,
    ...
    .bind(userId, tenantId, 'user@example.com', userPassword, 'Sample User', 'editor', 'active')
    .run()
  ...
  `,
    ...
    .bind(collectionId, tenantId, 'Articles', 'articles', 'Blog articles collection')
    .run()
  ...
  for (const field of fields) {
    ...
    `,
      ...
      .bind(fieldId, collectionId, field.name, field.type, field.required)
      .run()
  }
  ...
  for (const article of sampleArticles) {
    ...
    `,
      ...
      .bind(dataId, collectionId, tenantId, JSON.stringify(article), adminId)
      .run()
  }
  ...
  `,
    ...
    .bind(apiKeyId, tenantId, 'Sample API Key', hashedKey, JSON.stringify(['read', 'write']))
    .run()
