...
import { PlanType, QUOTA_PRESETS } from '../utils/quota-presets'
 ...
 */
async function getTenantQuotas(
  c: Context,
  tenantId: string,
): Promise<{
 ...
 * @returns 当前计数值
 */
async function incrementCounter(c: Context, key: string, ttlMs: number): Promise<number> {
  // 优先使用 KV 存储（如果可用）
  if (c.env.KV) {
    try {
  ...
  try {
    ...
    if (usage) {
      ...
      `,
        ...
        .bind(requestsToday, resetDate, now, now, tenantId)
        .run()
    } else {
      // 如果使用量记录不存在，创建一条
      await c.env.DB.prepare(
      ...
      `,
        ...
        .bind(`usage_${tenantId}`, tenantId, today, now, now)
        .run()
    }
  } catch (error) {
