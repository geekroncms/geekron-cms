...
function wrapDatabaseError(error: any, operation: string, tenantId: string): Error {
  const wrapped = new Error(`[TenantIsolation] ${operation} failed for tenant ${tenantId}: ${error.message}`);
...
export function getSafeDB(c: Context): TenantSafeDB {
  ...
  if (!safeDB) {
    throw new Error('Tenant isolation middleware not initialized. Call getSafeDB() after tenantIsolationMiddleware.');
  }
  return safeDB;
}
