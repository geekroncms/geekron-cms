...
export const apiKeysRoutes = new Hono<{ Bindings: Bindings; Variables: Variables }>()
...
apiKeysRoutes.post('/', zValidator('json', createApiKeySchema), async (c) => {
  ...
  `,
    ...
    .bind(keyId, tenantId, name, hashedKey, JSON.stringify(permissions), expiresAt || null, now)
    .run()
