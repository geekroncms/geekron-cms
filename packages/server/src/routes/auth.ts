...
async function getUserTenants(db: any, userId: string) {
  ...
  const directTenants: any[] = await db
    .prepare(
      `
...
authRoutes.post('/login', zValidator('json', loginSchema), async (c) => {
  ...
  // Get tenant info
  const tenant: any = await db
    .prepare('SELECT id, name, slug, plan, status FROM tenants WHERE id = ?')
    .bind(targetTenantId)
    .first()
...
authRoutes.post('/register', zValidator('json', registerSchema), async (c) => {
  ...
  const existingTenant: any = await db
  ...
  `,
    ...
    .bind(userId, email, hashedPassword, name, now, now)
    .run()
  ...
  `,
    ...
    .bind(tenantId, tenantName, tenantSlug, email, now, now)
    .run()
  ...
  `,
    ...
    .bind(crypto.randomUUID(), tenantId, userId, now)
    .run()
...
authRoutes.post('/refresh', async (c) => {
  ...
  try {
    ...
    const user: any = await c.env.DB.prepare('SELECT status FROM users WHERE id = ?')
      .bind(userId)
      .first()
    ...
    const tenant: any = await c.env.DB.prepare('SELECT status FROM tenants WHERE id = ?')
      .bind(tenantId)
      .first()
