...
export const collectionDataRoutes = new Hono();
...
collectionDataRoutes.post('/', zValidator('json', createDataSchema), async (c) => {
  ...
  await c.env.DB.prepare(`
    ...
    VALUES (?, ?, ?, ?, ?, ?, ?, ?)
  `).bind(dataId, collectionId, tenantId, JSON.stringify(data), userId, userId, now, now).run();

  // Fetch the created record
  const created: any = await c.env.DB.prepare(
    'SELECT * FROM collection_data WHERE id = ?'
  ).bind(dataId).first();
...
collectionDataRoutes.get('/:collectionId', zValidator('query', querySchema), async (c) => {
  ...
  const results: any = await c.env.DB.prepare(query).bind(...params).all();
...
collectionDataRoutes.get('/:collectionId/:id', async (c) => {
  ...
  `).bind(dataId, collectionId, tenantId).first();
 ...
 * Update a data entry
 */
collectionDataRoutes.patch('/:collectionId/:id', zValidator('json', updateDataSchema), async (c) => {
  const tenantId = c.get('tenantId');
  const userId = c.get('userId');
  const collectionId = c.req.param('collectionId');
  const dataId = c.req.param('id');
  ...
  await c.env.DB.prepare(`
    ...
    WHERE id = ?
  `).bind(JSON.stringify(mergedData), userId, now, dataId).run();
...
collectionDataRoutes.delete('/:collectionId/:id', async (c) => {
  ...
  await c.env.DB.prepare(`
    ...
    WHERE id = ? AND collection_id = ? AND tenant_id = ?
  `).bind(dataId, collectionId, tenantId).run();
...
collectionDataRoutes.post('/:collectionId/bulk', async (c) => {
  ...
  for (const item of items) {
    ...
    await c.env.DB.prepare(`
      INSERT INTO collection_data (id, collection_id, tenant_id, data, created_by, updated_by, created_at, updated_at)
      VALUES (?, ?, ?, ?, ?, ?, ?, ?)
    `).bind(dataId, collectionId, tenantId, JSON.stringify(item), userId, userId, now, now).run();
