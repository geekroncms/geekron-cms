...
const createSchemaSchema = z.object({
  ...
  version: z.string().regex(/^\d+\.\d+\.\d+$/, 'Version must be semver format').default('1.0.0'),
  schema_json: z.record(z.any(), 'Schema must be a valid JSON object'),
...
const createFieldSchema = z.object({
  ...
  field_type: z.enum(['text', 'number', 'boolean', 'date', 'json', 'relation', 'email', 'url', 'phone']),
...
const updateFieldSchema = z.object({
  field_name: z.string().min(1).max(50).regex(/^[a-zA-Z_][a-zA-Z0-9_]*$/).optional(),
  field_type: z.enum(['text', 'number', 'boolean', 'date', 'json', 'relation', 'email', 'url', 'phone']).optional(),
...
function validateFieldConfig(fieldType: string, config: any): { valid: boolean; error?: string } {
  ...
  if (['text', 'email', 'url', 'phone'].includes(fieldType)) {
    ...
    if (config.minLength !== undefined && (typeof config.minLength !== 'number' || config.minLength < 0)) {
...
async function getSchemaById(db: D1Database, schemaId: string, tenantId: string): Promise<any> {
  const schema: any = await db.prepare(
    'SELECT * FROM metadata_schemas WHERE id = ? AND tenant_id = ?'
...
async function getFieldById(db: D1Database, fieldId: string, schemaId: string): Promise<any> {
  const field: any = await db.prepare(
    'SELECT * FROM metadata_fields WHERE id = ? AND schema_id = ?'
  ).bind(fieldId, schemaId).first();
...
metadataRoutes.get('/schemas', async (c) => {
  ...
  try {
    ...
    `).bind(...params.slice(0, params.length - 2)).first();
    ...
    throw new ApiError('Failed to fetch schemas', 500);
  }
});
...
metadataRoutes.get('/schemas/:id', async (c) => {
  ...
  return c.json({
    ...schema,
    schema_json: typeof schema.schema_json === 'string' ? JSON.parse(schema.schema_json) : schema.schema_json,
    fields: (fields.results || []).map((f: any) => ({
      ...f,
      field_config: typeof f.field_config === 'string' ? JSON.parse(f.field_config) : f.field_config,
    })),
...
metadataRoutes.post('/schemas', zValidator('json', createSchemaSchema), async (c) => {
  ...
  if (!validation.valid) {
    throw new ApiError(validation.error || 'Invalid schema format', 400);
  }
  ...
  if (existing) {
    throw new ApiError('Schema name already exists', 409);
  }
  ...
  try {
    ...
    throw new ApiError('Failed to create schema', 500);
  }
});
...
metadataRoutes.patch('/schemas/:id', zValidator('json', updateSchemaSchema), async (c) => {
  ...
  if (body.name && body.name !== existing.name) {
    ...
    ).bind(tenantId, body.name, id).first();
...
metadataRoutes.delete('/schemas/:id', async (c) => {
  ...
  try {
    await c.env.DB.prepare(
      'DELETE FROM metadata_schemas WHERE id = ?'
    ).bind(id).run();
...
metadataRoutes.post('/schemas/:id/fields', zValidator('json', createFieldSchema), async (c) => {
  ...
  ).bind(schemaId, body.field_name).first();
...
metadataRoutes.patch('/fields/:fieldId', zValidator('json', updateFieldSchema), async (c) => {
  ...
  const field: any = await c.env.DB.prepare(
    'SELECT * FROM metadata_fields WHERE id = ?'
  ).bind(fieldId).first();
  ...
  if (body.field_config) {
    const configValidation = validateFieldConfig(body.field_type || field.field_type, body.field_config);
    if (!configValidation.valid) {
      throw new ApiError(configValidation.error || 'Invalid field config', 400);
    }
  }
  ...
  if (body.field_name && body.field_name !== field.field_name) {
    ...
    ).bind(field.schema_id, body.field_name, fieldId).first();
...
metadataRoutes.delete('/fields/:fieldId', async (c) => {
  ...
  // 获取字段信息
  const field: any = await c.env.DB.prepare(
    'SELECT schema_id FROM metadata_fields WHERE id = ?'
  ).bind(fieldId).first();
  ...
  try {
    await c.env.DB.prepare(
      'DELETE FROM metadata_fields WHERE id = ?'
    ).bind(fieldId).run();
