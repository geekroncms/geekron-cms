...
import { z } from 'zod'
import { checkResourceQuota, getQuotaStatus, updateResourceUsage } from '../middleware/quota-check'
import { ApiError } from '../utils/errors'
import { PlanType, getQuotaPreset } from '../utils/quota-presets'

export const quotaRoutes = new Hono<{ Bindings: Bindings; Variables: Variables }>
  ...
  try {
    ...
    return c.json({
      data: {
        ...
        usage_percent: {
          requests_today:
            Math.round((usage.requests_today / quotas.max_requests_per_day) * 10000) / 100,
  ...
  try {
    ...
    `,
      ...
      .bind(
        ...
        JSON.stringify({
          reason: body.reason,
          reset_requests: body.reset_requests,
          reset_storage: body.reset_storage,
        }),
        now,
      )
      .run()
