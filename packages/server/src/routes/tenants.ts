...
const createTenantSchema = z.object({
  ...
  subdomain: z
    ...
    .regex(/^[a-z][a-z0-9-]*[a-z0-9]$/, 'Subdomain must start and end with lowercase letter, and contain only lowercase letters, numbers, and hyphens'),
...
function checkTenantStatus(tenant: any) {
  ...
  if (tenant.status === 'deleted') {
    throw new ApiError('Tenant has been deleted', 410);
  }
  
  if (tenant.status === 'suspended') {
    throw new ApiError('Tenant is suspended', 403);
  }
}
...
tenantRoutes.post('/', zValidator('json', createTenantSchema), async (c) => {
  ...
  if (!available) {
    throw new ApiError('Subdomain already taken', 409);
  }
...
tenantRoutes.delete('/:id', async (c) => {
  ...
  if (existing.status === 'deleted') {
    throw new ApiError('Tenant already deleted', 400);
  }
...
tenantRoutes.post('/:id/activate', async (c) => {
  ...
  if (existing.status === 'deleted') {
    throw new ApiError('Cannot activate a deleted tenant', 400);
  }
...
tenantRoutes.post('/:id/suspend', async (c) => {
  ...
  if (existing.status === 'deleted') {
    throw new ApiError('Cannot suspend a deleted tenant', 400);
  }
