...
userRoutes.post('/', zValidator('json', createUserSchema), async (c) => {
  ...
  `,
    ...
    .bind(userId, body.email, hashedPassword, body.name, body.role, now, now)
    .run()
  ...
  `,
    ...
    .bind(crypto.randomUUID(), tenantId, userId, body.role, now)
    .run()
...
userRoutes.post('/invite', zValidator('json', inviteUserSchema), async (c) => {
  ...
  `,
    ...
    .bind(memberId, tenantId, user.id, role, JSON.stringify(permissions || []), userId, now)
    .run()
...
userRoutes.patch('/:id', zValidator('json', updateUserSchema), async (c) => {
  ...
  const existing: any = await c.env.DB.prepare('SELECT id FROM users WHERE id = ?')
    .bind(userId)
    .first()
  ...
  if (body.role) {
    ...
    `,
      ...
      .bind(body.role, userId, tenantId)
      .run()
  }
...
userRoutes.post('/change-password', zValidator('json', changePasswordSchema), async (c) => {
  ...
  const user: any = await c.env.DB.prepare('SELECT password FROM users WHERE id = ?')
    .bind(userId)
    .first()
