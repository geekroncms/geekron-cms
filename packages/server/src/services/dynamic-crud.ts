...
import { buildZodSchema, loadSchemaMetadata } from '../middleware/data-validation'
 ...
 */
async function getCachedSchemaMetadata(db: any, kv: any, schemaId: string, tenantId: string) {
...
dynamicCrudRoutes.get('/:schemaId', schemaMiddleware, async (c) => {
  ...
  const results: any = await db
    .prepare(queryResult.sql)
    .bind(...queryResult.params)
    .all()
 ...
 * Create a new record
 */
dynamicCrudRoutes.post('/:schemaId', schemaMiddleware, validateBodyMiddleware, async (c) => {
  const tenantId = c.get('tenantId')
  ...
  await db
    ...
    .bind(recordId, schemaId, tenantId, JSON.stringify(data), userId, userId, now, now)
    .run()
 ...
 * Update a record
 */
dynamicCrudRoutes.patch('/:schemaId/:id', schemaMiddleware, validateBodyMiddleware, async (c) => {
  const tenantId = c.get('tenantId')
  const userId = c.get('userId')
  const schemaId = c.req.param('schemaId')
  const id = c.req.param('id')
  ...
  await db
    ...
    .bind(JSON.stringify(mergedData), userId, now, id)
    .run()
 ...
 * Bulk create records
 */
dynamicCrudRoutes.post('/:schemaId/bulk', schemaMiddleware, async (c) => {
  const tenantId = c.get('tenantId')
  const userId = c.get('userId')
  const schemaId = c.req.param('schemaId')
  const db = c.env.DB
  ...
  for (let i = 0; i < items.length; i++) {
    ...
    await db
      ...
      .bind(recordId, schemaId, tenantId, JSON.stringify(result.data), userId, userId, now, now)
      .run()
