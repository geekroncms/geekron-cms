 ...
 */
export function buildSelectClause(select: string[] | undefined, tableAlias?: string): string {
  if (!select || select.length === 0) {
    return tableAlias ? `${tableAlias}.*` : '*'
  }

  return select.map((field) => (tableAlias ? `${tableAlias}.${field}` : field)).join(', ')
}
...
export function buildDataQuery(
  ...
  const { filters = {}, sort, order = 'asc', select, joins = [], page = 1, limit = 20 } = options

  const selectClause = buildSelectClause(select, tableAlias)
  const { clause: whereClause, params: whereParams } = buildWhereClause(filters, tableAlias)
