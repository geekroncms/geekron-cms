...
**问题**: `createAuthHeaders()`
是 async 函数，但在很多测试调用中没有 await，导致传递的是 Promise 对象而不是实际的 headers 对象。
...
**问题**: Mock DB 的 `first()` 方法无法正确处理 `tenant_members`
表的 COUNT 查询。
...
- `errorHandler()`
  中间件注册在 app 级别，但子路由（api、protectedApi）是新的 Hono 实例，没有继承错误处理器
...
- 在 `ApiError` 构造函数中添加 `Object.setPrototypeOf(this, ApiError.prototype)`
  确保 prototype 链正确
...
**问题**: `tenantMiddleware`
拒绝了所有非 active 状态的租户，但某些路由（如 activate/suspend/delete）需要能够操作这些状态的租户。
