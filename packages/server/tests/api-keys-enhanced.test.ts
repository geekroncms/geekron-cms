...
describe('API Key Enhanced Features', () => {
  ...
  describe('API Key Rotation', () => {
    ...
    test('should generate new key with correct format on rotation', () => {
      const newKey = 'gk_' + Array.from(mockCrypto.getRandomValues(new Uint8Array(32)))
        .map(b => b.toString(16).padStart(2, '0'))
        .join('');
  ...
  describe('API Key Usage Statistics', () => {
    ...
    test('should calculate usage statistics', () => {
      ...
      expect(usageData.readCount + usageData.writeCount + usageData.deleteCount)
        .toBe(usageData.totalRequests);
    });
  });
});
