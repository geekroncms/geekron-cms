...
describe('Tenant Isolation Middleware', () => {
  describe('SQL Injection - SELECT queries', () => {
    ...
    test('should handle complex SELECT with multiple clauses', () => {
      const sql =
        'SELECT c.*, COUNT(f.id) as field_count FROM collections c LEFT JOIN collection_fields f ON c.id = f.collection_id WHERE c.id = ? GROUP BY c.id ORDER BY c.created_at DESC LIMIT 10'
      const result = injectSelectConstraint(sql, 'tenant-123')
  ...
  describe('Edge cases and security', () => {
    ...
    test('should handle subqueries correctly', () => {
      const sql =
        'SELECT * FROM collections WHERE id IN (SELECT collection_id FROM collection_fields WHERE type = ?)'
      const result = injectSelectConstraint(sql, 'tenant-123')
...
describe('Tenant Isolation - Real-world SQL patterns', () => {
  test('should handle JOIN queries', () => {
    const sql =
      'SELECT c.*, u.name FROM collections c JOIN users u ON c.created_by = u.id WHERE c.id = ?'
    const result = injectSelectConstraint(sql, 'tenant-123')
  ...
  test('should handle LEFT JOIN queries', () => {
    const sql =
      'SELECT c.*, COUNT(f.id) as field_count FROM collections c LEFT JOIN collection_fields f ON c.id = f.collection_id WHERE c.tenant_id = ? GROUP BY c.id'
    const result = injectSelectConstraint(sql, 'tenant-123')
